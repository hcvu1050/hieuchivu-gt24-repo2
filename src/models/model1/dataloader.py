...
def load_data (sample_train: float = None, feature_info = True):
    ...
    if sample_train is not None:
        ...
        sampled_train_dataset = train_dataset.shuffle(buffer_size=len(train_dataset), seed=RANDOM_STATE).take(num_samples)
        
    if feature_info:
        info = {
            'X_group_num_features' : train_dataset.element_spec[0][INPUT_GROUP_LAYER_NAME].shape[1],
            'X_technique_num_features' : train_dataset.element_spec[0][INPUT_TECHNIQUE_LAYER_NAME].shape[1]
        }
        return sampled_train_dataset, cv_dataset, test_dataset, info
    
    return sampled_train_dataset, cv_dataset, test_dataset